...
def setJointPosition(incAngle, steps):
    for i  in range(steps):
       #errorCode=vrep.simxSetJointTargetPosition(clientID, Prismatic_joint_handle, i*incAngle, vrep.simx_opmode_oneshot_wait)
        #errorCode=vrep.simxSetJointTargetVelocity(clientID,Prismatic_joint_handle,2, vrep.simx_opmode_oneshot_wait)
        errorCode=vrep.simxSetJointPosition(clientID, Prismatic_joint_handle, 0.11-i*incAngle, vrep.simx_opmode_oneshot_wait)
# 每步 10 度, 轉兩圈
setJointPosition(0.005, 100)
# 每步 1 度, 轉兩圈
